...
class Conv2dBlock(nn.Module):
    def __init__(self, in_dim, out_dim, ks, st, padding=0,
                 ...
                 use_bias=True, activation_first=True):
...
class Decoder(nn.Module):
    def __init__(self, ups=3, n_res=2, dim=2, out_dim=1, res_norm='adain', activ='relu', pad_type='reflect'):
        ...
        for i in range(ups):
            ...
            self.con= Conv2dBlock(dim, dim // 2, 5, 1, 2, norm='in', activation=activ, pad_type=pad_type)
            #dim //= 2
        
        # Final Convolutional Block
        self.con_final= Conv2dBlock(dim, out_dim, 7, 1, 3, norm=res_norm, activation='tanh', pad_type=pad_type)
...
class GenModel_FC(nn.Module):
    ...
    def mix(self, feat_xs, feat_embed):
            ...
            feat_mix = torch.cat(
                [feat_xs, dowm_sampling_embed], dim=1
            )  # b,1024,8,27
