...
from parameters import vocab,encoder,decoder
CREATE_PAIRS = False
...
class IAM_words(D.Dataset):
    ...
    def read_image_single(self, file_name):
        ...
        scratch_path="/scratch/iwi_5120h_exps/research_gans/Words_image"
        url = os.path.join(scratch_path,os.path.join(elements[0], f"{elements[0]}-{elements[1]}", f"{elements[0]}-{elements[1]}-{elements[2]}-{elements[3]}")+".png")
    ...
    def label_padding(self, labels, num_tokens):
        pad_lbl=equlize_lbl(labels)
        ll = [letter2index[i] for i in pad_lbl]
