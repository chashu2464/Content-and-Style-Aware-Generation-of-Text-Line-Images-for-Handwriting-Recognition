...
class TextDataset:
    ...
    def __getitem__(self, index):
        ...
        real_img = self.transform(
            self.IMG_DATA_AUTHOR[rand_id_real]["img"].convert("L")
        )
        ...
        # for lbl in labels:
        #     self.max_str=len(lbl) if len(lbl)> self.max_str else self.max_str
        # Max_str=self.max_str
        # print("Maxium string",Max_str)
        # max_str=len(max([str1 for str1 in labels ]))
        # print("max string len: ",max_str)
        imgs_pad = []
        imgs_wids = []
        ...
        for img in imgs:
            ...
            padded_image = padded_image[:, :max_width]

            # outImg = np.zeros(( img_height, max_width), dtype='float32')
            # outImg[:, :img_width] = img[:, :max_width]

            imgs_pad.append(self.transform((Image.fromarray(padded_image))))
