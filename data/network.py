    ...
    def forward(self, train_data_list, epoch, mode, cer_func=None):
        ...
        if mode == "rec_update":
            ...
            pred_xt_tr = self.rec(
                tr_img_rec,
                tr_label_rec,
                img_width=torch.from_numpy(np.array([IMAGE_WIDTH] * batch_size)),
            )
            tr_label_rec2 = tr_label_rec[:, 1:]  # remove <GO>
            l_rec_tr = crit(
                log_softmax(pred_xt_tr.reshape(-1, len(vocab))),
            ...
            if self.iter_num % self.show_iter_num == 0:
                with torch.no_grad():
                    pred_xt = self.rec(
                        xg,
                        label_xt,
                        img_width=torch.from_numpy(
                            np.array([IMAGE_WIDTH] * batch_size)
                        ),
                    )
                    pred_xt_swap = self.rec(
                        xg_swap,
